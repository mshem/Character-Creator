...
namespace MalbersAnimations
{
    ...
    public class MInput : MonoBehaviour, IInputSource , IAnimatorListener
    {
        ...
        {
            var strafe = new InputRow(true, "Sneak", "Sneak", KeyCode.C, InputButton.Down, InputType.Key);

            inputs.Add(strafe);

            //var method = this.GetUnityAction<ScriptableObject>("MAnimal", "Stance_Toggle");

            //var sneak = MTools.GetInstance<ScriptableObject>("Sneak");

            //if (method != null && sneak!=null) UnityEditor.Events.UnityEventTools.AddObjectPersistentListener<ScriptableObject>(strafe.OnInputDown, method, sneak);
            //MTools.SetDirty(this);
        }
        ...
        [ContextMenu("Create/Lock On Target Cycle")]
        private void CreateLockOnTargetCycle()
        {
            var key = new InputRow(true, "Cycle Targets", "Cycle Targets", KeyCode.Tab, InputButton.Down, InputType.Key);
            inputs.Add(key);

            var method = this.GetUnityAction("LockOnTarget", "CycleTargets");
            if (method != null) UnityEditor.Events.UnityEventTools.AddPersistentListener(key.OnInputDown, method);
