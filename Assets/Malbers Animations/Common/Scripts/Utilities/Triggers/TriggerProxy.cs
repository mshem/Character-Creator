...
namespace MalbersAnimations.Utilities
{
    ...
    public class TriggerProxy : MonoBehaviour, IMLayer
    {
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            {
                var roott = other.transform.root.gameObject;        //Get the animal on the entering collider

                if (!MTools.Layer_in_LayerMask(roott.layer, Layer)) //If the Root is not on the same Layer
                {
                    //Means the Root is not on the real root since its not on the search layer
                    roott = FindRealParent(other.transform);
                    // Debug.Log("roott = " + roott);
                }
                ...
                if (EnteringGameObjects.Contains(roott))
                {
                    return;
                }
                else
                {
                    EnteringGameObjects.Add(roott);

                    EnterTriggerInteraction(roott, other);

                    OnGameObjectEnter.Invoke(roott);

                    if (m_debug) Debug.Log($"<b>{name}</b> [Entering GameObject] -> [{roott.name}]", this);
                }
            }
        }
        ...
        void OnTriggerExit(Collider other)
        {
            if (TrueConditions(other))
            {
                ...
                var roott = other.transform.root.gameObject;         //Get the gameObject on the entering collider
                ...
                if (EnteringGameObjects.Contains(roott))             //Means that the Entering GameObject still exist
                {
                    if (!m_colliders.Exists(c => c != null && c.transform.root.gameObject == roott)) //Means that all that root colliders are out
                    {
                        EnteringGameObjects.Remove(roott);
                        OnGameObjectExit.Invoke(roott);

                        ExitTriggerInteraction(roott, other);

                        if (m_debug) Debug.Log($"<b>{name}</b> [Leaving Gameobject] -> [{roott.name}]", this);
                    }
                }
                CheckMissingColliders();
            }
